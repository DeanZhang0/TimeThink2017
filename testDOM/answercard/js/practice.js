...
//    发布的方法
    releaseFun: function (e) {
        ...
        for (var i = 0; i < allAnswer.length; i++) {
            ...
            var point = $(allAnswer[i]).find("[data-card=cs-t-fraction]").text();
    ...
    buildBottomFun: function () {
        ...
        for (index in currentData) {
            ...
            if (currentData[index].type == "judge") {
                eachSubject.find(".analy-b-correct").text(this.judgeType[currentAnswer[0] - 1]);
            } else {
                temAnswer = "";
